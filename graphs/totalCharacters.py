...
fig1.canvas.set_window_title('Total Character Count by User')
...
query = """
  SELECT sender_name,SUM(LENGTH(content)) as count
  FROM Messages
  GROUP BY sender_name
  ORDER BY count DESC
  """
# ---Generate top TOPUSERS number of users and charCount lists
# read sql query into dataframe
messageFrame = pd.read_sql_query(query, connection)
connection.close()
users = messageFrame["sender_name"].tolist()[:TOPUSERS]
charCount = messageFrame["count"].tolist()[:TOPUSERS]
...
# ---plot data---
ax1.bar(usersIndexes, charCount,
